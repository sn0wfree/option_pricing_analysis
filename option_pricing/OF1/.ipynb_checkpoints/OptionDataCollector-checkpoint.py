...
class Collector():
    ...
    def beforeStart(self, TargetAssetsCode=510050):
        ...
        self.watchlist = pd.DataFrame([(c, cp, k) for k, v in sssO.items(
        ) for cp, v2 in v.items() for c in v2], columns=['SinaCode', 'OptionType', 'YM'])
    ...
    def regularGetRTOQ(self, watchlist,RequestTimeStamp, repeatfreq=None, endtime=111111111111111111111111, merge=True, Optimizer=False):
        # self.task=set(watchlist)
        task = set(watchlist)
        if repeatfreq != None:
            while time.time.now() > endtime + 1:
                df = self.api.getRealtimeOptionQuotes(task, RequestTimeStamp)
                if merge:
...
def main(cc, TargetAssetsCode, iDB, force, inmemory=True, savepath='data/'):
    ...
    while True:
        if force != 0 and force != False:
            print 'You choose  running program forcely, will run %d times left' % force
            ...
            if force == 0:
                ...
                optionname = '50ETF'
                n = datetime.datetime.now()
                with DBConnect().SqliteConnection(savepath + 'Option-%s-%s.sqlite' % (optionname, n.strftime('%Y-%m-%d'))) as discconn:
