...
class MockBackTest(DerivativeSummary):
    def __init__(self, uri, trial_table, db='mock_op_backtest',
                 contract_2_person_rule={'MO\d{4}-[CP]-[0-9]+.CFE': 'll', }):
        node_local = BaseSingleFactorTableNode(uri)
        sql = f'select * from {db}.{trial_table} '
...
if __name__ == '__main__':
    ...
    PR = MockBackTest(
        uri, 'trial_1', db='mock_op_backtest',
        contract_2_person_rule={'MO\d{4}-[CP]-[0-9]+.CFE': 'll', }

    )
    PR.auto(wh, 'trail_1.xlsx',
            output_config={'分品种': 'ALL', '期货多头': ['ll'], '期货对冲': ['IM'], '汇总': ['ll']},
            start_with='2022-09-04', trade_type_mark={"卖开": 1, "卖平": -1,
                                                      "买开": 1, "买平": -1,
                                                      "买平今": -1, })
    print(1)
