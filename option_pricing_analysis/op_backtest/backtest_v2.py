...
class FastRunBackTest(object):
    ...
    @classmethod
    def fast(cls, strategy_result, transactions, quote, lastdel_multi_hedge):
    ...
    @staticmethod
    def status_to_transcations(dd, col_rename={'dt': '报单日期', 'contract_code': '委托合约', 'fee': '成交均价'}):
...
def single_run(otm_selected, hedge_size):
    op_portfolio = OptionPortfolioWithDT(otm_selected)
    Delta, Gamma, Vega, Theta, Rho = op_portfolio.create_greek_matrix_all()
    # res = OptBundle.run_opt(initial_weights, otm_selected, hedge_size, Delta, Gamma, Vega, Rho, method='SLSQP')
    res = op_portfolio.run_opt(hedge_size, Delta, Gamma, Vega, Theta, Rho, verbose=False)
...
if __name__ == '__main__':
    ...
    full_greek_caled_marked_60m = load_full_greek_data(name='full_greek_caled_marked_60m.parquet')
    ...
    h = boost_up(single_run_v2, create_tasks_data(full_greek_caled_marked_60m, hedge_size, start_dt), star=True,
                 core=19)
    strategy_result = pd.concat(h)

    lastdel_multi_hedge = pd.read_excel('lastdel_multi_hedge.xlsx')
    # lastdel_multi_hedge = lastdel_multi_hedge.set_index('委托合约')
    ...
    transactions = FastRunBackTest.strategy_result_2_transcation(strategy_result, zz1000_60m)
